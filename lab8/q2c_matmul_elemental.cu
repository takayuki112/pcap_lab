...
__global__ void matmul_ele(int *a, int *b, int *c, int ha, int wahb, int wb){
...
int main(void){
    ...
    int blocksPerGrid = 1;
    dim3 threadsPerBlock(wb, ha, 1);
    
    matmul_ele<<<blocksPerGrid, threadsPerBlock>>>(da, db, dc, ha, wahb, wb);
