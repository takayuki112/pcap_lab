...
__global__ void matmul_rows(int *a, int *b, int *c, int ha, int wahb, int wb){
...
int main(void){
    ...
    int blocksPerGrid = 1;
    int threadsPerBlock = ha;
    
    matmul_rows<<<blocksPerGrid, threadsPerBlock>>>(da, db, dc, ha, wahb, wb);
